...
from mars_time.retimers import datetime_to_mars_time, MarsTime
...
def find_aphelion(mars_year: int) -> MarsTime:
    ...
    def find_days_since_j2000_of_aphelion(day: float) -> float:
        day = float(day)
        pre_aphleion = datetime_to_mars_time(j2000 + datetime.timedelta(days=day - 0.001))
        post_aphelion = datetime_to_mars_time(j2000 + datetime.timedelta(days=day + 0.001))
    ...
    return datetime_to_mars_time(j2000 + datetime.timedelta(days=days))


def find_perihelion(mars_year: int) -> MarsTime:
    """Find the MarsTime of perihelion for a given Mars year.
    ...
    def find_days_since_j2000_of_perihelion(day: float) -> float:
        day = float(day)
        pre_perihelion = datetime_to_mars_time(j2000 + datetime.timedelta(days=day - 0.001))
        post_perihelion = datetime_to_mars_time(j2000 + datetime.timedelta(days=day + 0.001))
